...
public:
    ...
    * @param frame the object from which to take depth and imgMat data.
    ...
    * @param depth the depth imgMat.
    * @param imgMat the color imgMat (only for ColorTSDF).
      For color TSDF a depth data should be registered with color data, i.e. have the same intrinsics & camera pose.
      This can be done using function registerDepth() from 3d module.
    * @param pose the pose of camera in global coordinates.
    */
    CV_WRAP_AS(integrateColor) void integrate(InputArray depth, InputArray imgMat, InputArray pose);

    // Python bindings do not process noArray() default argument correctly, so the raycast() method is repeated several times

    /** @brief Renders the volume contents into an imgMat. The resulting points and normals are in camera's coordinate system.

    Rendered imgMat size and camera intrinsics are taken from volume settings structure.

    * @param cameraPose the pose of camera in global coordinates.
    * @param points imgMat to store rendered points.
    * @param normals imgMat to store rendered normals corresponding to points.
    */
    ...
    * @param points imgMat to store rendered points.
    * @param normals imgMat to store rendered normals corresponding to points.
    * @param colors imgMat to store rendered colors corresponding to points (only for ColorTSDF).
    */
    ...
    * @param cameraPose the pose of camera in global coordinates.
    * @param height the height of result imgMat
    * @param width the width of result imgMat
    ...
    * @param normals the storage of normals (corresponding to input points) in the imgMat.
    */
