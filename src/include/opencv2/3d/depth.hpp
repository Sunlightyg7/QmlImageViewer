...
/** Object that can compute the normals in an imgMat.
 ...
 * - the normals with bilateral filtering on a depth imgMat from
...
public:
     ...
     * @param rows the number of rows of the depth imgMat normals will be computed on
     * @param cols the number of cols of the depth imgMat normals will be computed on
    ...
    /** Given a set of 3d points in a depth imgMat, compute the normals at each point.
 ...
 * @param outputImagePlaneSize the imgMat plane dimensions of the external camera (width, height)
...
/** Converts a depth imgMat to 3d points. If the mask is empty then the resulting array has the same dimensions as `depth`,
 * otherwise it is 1d vector containing mask-enabled values only.
 * The coordinate system is x pointing left, y down and z away from the camera
 * @param depth the depth imgMat (if given as short int CV_U, it is assumed to be the depth in millimeters
...
/** If the input imgMat is of type CV_16UC1 (like the Kinect one), the imgMat is converted to floats, divided
 * by depth_factor to get a depth in meters, and the values 0 are converted to std::numeric_limits<float>::quiet_NaN()
 * Otherwise, the imgMat is simply converted to floats
 * @param in the depth imgMat (if given as short int CV_U, it is assumed to be the depth in millimeters
 *              (as done with the Microsoft Kinect), it is assumed in meters)
 * @param type the desired output depth (CV_32F or CV_64F)
 * @param out The rescaled float depth imgMat
...
/** Warps depth or RGB-D imgMat by reprojecting it in 3d, applying Rt transformation
 * and then projecting it back onto the imgMat plane.
 ...
 * @param imgMat RGB imgMat (optional), should be 1-, 3- or 4-channel CV_8U
 ...
 * @param warpedImage The warped RGB imgMat (optional)
 * @param warpedMask The mask of valid pixels in warped imgMat (optional)
 */
CV_EXPORTS_W void warpFrame(InputArray depth, InputArray imgMat, InputArray mask, InputArray Rt, InputArray cameraMatrix,
...
/** Find the planes in a depth imgMat
 * @param points3d the 3d points organized like the depth imgMat: rows x cols with 3 channels
 * @param normals the normals for every point in the depth imgMat; optional, can be empty
 * @param mask An imgMat where each pixel is labeled with the plane it belongs to
