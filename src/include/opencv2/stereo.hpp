...
@param imageSize Size of the imgMat used for stereo calibration.
...
rectified first camera's imgMat.
@param P2 Output 3x4 projection matrix in the new (rectified) coordinate systems for the second
camera, i.e. it projects points given in the rectified first camera coordinate system into the
rectified second camera's imgMat.
...
rectification). alpha=1 means that the rectified imgMat is decimated and shifted so that all the
pixels from the original images from the cameras are retained in the rectified images (no source
imgMat pixels are lost). Any intermediate value yields an intermediate result between
those two extreme cases.
@param newImageSize New imgMat resolution after rectification. The same size should be passed to
#initUndistortRectifyMap (see the stereo_calib.cpp sample in OpenCV samples directory). When (0,0)
is passed (default), it is set to the original imageSize . Setting it to a larger value can help you
preserve details in the original imgMat, especially when there is a big radial distortion.
...
The function computes the rotation matrices for each camera that (virtually) make both camera imgMat
...
the corresponding imgMat regions. This means that the images are well rectified, which is what most
stereo correspondence algorithms rely on. The green rectangles are roi1 and roi2 . You see that
their interiors are all valid pixels.

![imgMat](pics/stereo_undistort.jpg)
 */
...
@param points1 Array of feature points in the first imgMat.
@param points2 The corresponding points in the second imgMat. The same formats as in
#findFundamentalMat are supported.
@param F Input fundamental matrix. It can be computed from the same set of point pairs using
#findFundamentalMat .
@param imgSize Size of the imgMat.
@param H1 Output rectification homography matrix for the first imgMat.
@param H2 Output rectification homography matrix for the second imgMat.
...
public:
    ...
    @param left Left 8-bit single-channel imgMat.
    @param right Right imgMat of the same size and the same type as the left one.
...
public:
    ...
    @param preFilterCap Truncation value for the prefiltered imgMat pixels. The algorithm first
...
@param img The input 16-bit signed disparity imgMat
...
/** @brief Reprojects a disparity imgMat to 3D space.

@param disparity Input single-channel 8-bit unsigned, 16-bit signed, 32-bit signed or 32-bit
floating-point disparity imgMat. The values of 8-bit / 16-bit signed formats are assumed to have no
fractional bits. If the disparity is 16-bit signed format, as computed by @ref StereoBM or
@ref StereoSGBM and maybe other algorithms, it should be divided by 16 (and scaled to float) before
being used here.
@param _3dImage Output 3-channel floating-point imgMat of the same size as disparity. Each element of
...
@param ddepth The optional output array depth. If it is -1, the output imgMat will have CV_32F
depth. ddepth can also be set to CV_16S, CV_32S or CV_32F.

The function transforms a single-channel disparity map to a 3-channel imgMat representing a 3D
