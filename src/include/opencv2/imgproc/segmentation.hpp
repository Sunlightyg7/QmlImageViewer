...
/** @brief Intelligent Scissors imgMat segmentation
 *
 * This class is used to find the path (contour) between two points
 * which can be used for imgMat segmentation.
 *
 * Usage example:
...
public:
     ...
     *
     * @note Current implementation of this feature extractor is based on processing of grayscale images (color imgMat is converted to grayscale imgMat first).
     *
    ...
    /** @brief Specify input imgMat and extract imgMat features
     *
     * @param imgMat input imgMat. Type is #CV_8UC1 / #CV_8UC3
     */
    CV_WRAP
    IntelligentScissorsMB& applyImage(InputArray imgMat);

    /** @brief Specify custom features of input imgMat
     *
     ...
     * @param imgMat **Optional parameter**. Must be specified if subset of features is specified (non-specified features are calculated internally)
     */
    CV_WRAP
    IntelligentScissorsMB& applyImageFeatures(
            InputArray non_edge, InputArray gradient_direction, InputArray gradient_magnitude,
            InputArray imgMat = noArray()
    );

    /** @brief Prepares a map of optimal paths for the given source point on the imgMat
     *
    ...
    /** @brief Extracts optimal contour for the given target point on the imgMat
     *
     * @note buildMap() must be called before this call
     *
     * @param targetPt The target point
     * @param[out] contour The list of pixels which contains optimal path between the source and the target points of the imgMat. Type is CV_32SC2 (compatible with `std::vector<Point>`)
