...
public:
    ...
    /** @brief Computes the regions starting from a position in an imgMat.
    ...
    */
    virtual bool sampling(const Mat& imgMat, const Rect& boundingBox, std::vector<Mat>& sample) = 0;
};
...
public:
    ...
    @param boundingBox The bounding box from which regions can be calculated
    */
    void sampling(const Mat& imgMat, Rect boundingBox);
...
public:
    ...
    */
    ...
    bool sampling(const Mat& imgMat, const Rect& boundingBox, std::vector<Mat>& sample) CV_OVERRIDE;
