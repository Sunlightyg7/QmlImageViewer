...
public:
    ...
    /** @brief Set the size for the network input, which overwrites the input size of creating model. Call this method when the size of input imgMat does not match the input size when creating model
     *
     * @param input_size the size of the input imgMat
     */
    ...
    /** @brief Detects faces in the input imgMat. Following is an example output.

     * ![imgMat](pics/lena-face-detection.jpg)

     *  @param imgMat an imgMat to detect
     *  @param faces detection results stored in a 2D cv::Mat of shape [num_faces, 15]
     *  - 0-1: x, y of bbox top left corner
     *  - 2-3: width, height of bbox
     *  - 4-5: x, y of right eye (blue point in the example imgMat)
     *  - 6-7: x, y of left eye (red point in the example imgMat)
     *  - 8-9: x, y of nose tip (green point in the example imgMat)
     *  - 10-11: x, y of right corner of mouth (pink point in the example imgMat)
     *  - 12-13: x, y of left corner of mouth (yellow point in the example imgMat)
     *  - 14: face score
     */
    CV_WRAP virtual int detect(InputArray imgMat, OutputArray faces) = 0;
...
public:
    ...
    /** @brief Aligns detected face with the source input imgMat and crops it
     *  @param src_img input imgMat
     *  @param face_box the detected face result from the input imgMat
     *  @param aligned_img output aligned imgMat
     */
    CV_WRAP virtual void alignCrop(InputArray src_img, InputArray face_box, OutputArray aligned_img) const = 0;

    /** @brief Extracts face feature from aligned imgMat
     *  @param aligned_img input aligned imgMat
